...
- **Probar la visión en la Pi real**: `pip install -r backend/requirements-vision.txt` (⚠️ `mediapipe` NO tiene wheels para Python 3.13, solo 3.11/3.12 — recrear el `.venv` con `python3.11 -m venv .venv` si hace falta), enchufar la C930e, encender visión desde Ajustes. Calibrar `VISION_MIN_DISTANCE` y la constante `FACE_WIDTH_M`/`FOCAL_PX` de `vision.py` si la distancia estimada sale corrida (medir con cinta métrica a 1 m y comparar con lo que muestra el panel).
