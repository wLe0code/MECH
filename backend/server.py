...
async def trivia_answer(choice: str):
    ...
    # Revelar el resultado habla (y tarda): en otro hilo, como el resto.
    pregunta = trivia.current()
    if not trivia.is_asking() or pregunta is None or not (0 <= idx < len(pregunta["options"])):
        return {"ok": False, "reason": "Ahora mismo no hay ninguna pregunta esperando."}
    threading.Thread(target=mech.answer_trivia_from_panel, args=(idx,), daemon=True).start()
